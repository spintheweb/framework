...
export class STWLayout {
	...
	private _lex(): void {
		...
		for (const expression of this._wbll.matchAll(SYNTAX)) {
			if (expression.groups?.error !== undefined) {
				throw new SyntaxError(
					expression.input.slice(0, expression.index) + " ⋙" + expression.input.slice(expression.index),
				);
			}
			...
			if (pattern[0]) {
				...
				if (token.symbol[0] === "\\" && "aAs".includes(token.symbol[1])) { // \a, \A, \s
					...
					for (const attr of attributesString.matchAll(attrRegex)) {
						...
						if (leadingSpace.trim() !== "") {
							const errorIndex = attributesStringStartIndex + lastIndex +
								leadingSpace.indexOf(leadingSpace.trim());
							throw new SyntaxError(
								this._wbll.slice(0, errorIndex) + " ⋙" + this._wbll.slice(errorIndex),
							);
						}
					...
					if (trailingChars.trim() !== "") {
						const errorIndex = attributesStringStartIndex + lastIndex +
							trailingChars.indexOf(trailingChars.trim());
				...
				if (
					"fitvxyz".indexOf(token.symbol) != -1 && this.tokens.at(-1) &&
					"aA".indexOf(this.tokens.at(-1)?.symbol || "") != -1 && !this.tokens.at(-1)?.text
				) {
		...
		this.tokens.forEach((token) => {
			if (token.symbol === "b" && token.args[1] === "stw") {
				switch (token.args[2]) {
					case "search":
						this._acts |= ACTIONS.stwsearch;
						break;
					case "filter":
						this._acts |= ACTIONS.stwfilter;
						break;
					case "insert":
						this._acts |= ACTIONS.stwinsert;
						break;
					case "update":
						this._acts |= ACTIONS.stwupdate;
						break;
					case "delete":
						this._acts |= ACTIONS.stwdelete;
						break;
					case "submit":
						this._acts |= ACTIONS.stwsubmit;
						break;
					case "logon":
						this._acts |= ACTIONS.stwlogon;
						break;
					case "logoff":
						this._acts |= ACTIONS.stwlogoff;
						break;
					case "pwdreset":
						this._acts |= ACTIONS.stwpwdreset;
						break;
				}
			}
		});
	...
	public render(
		...
		if (!this._render) {
			...
			this._render = new Function("req", "session", "fields", "ph", "wbpl", fn) as (
				req: Request,
				session: STWSession,
				fields: string[],
				ph: Map<string, string>,
	...
	private initializeHandlers(): Map<string, (token: STWToken, inline: boolean) => string> {
		...
		handlers.set("i", (token) => {
			...
			if (mode === "1") {
				...
				srcExpression =
					`(()=>{const opts=${options};let idx=opts.findIndex((v,i)=>i%2===0&&v==String(fieldValue));return (idx!==-1&&opts[idx+1])?opts[idx+1]:opts.at(-1);})()`;
			} else {
		...
		handlers.set("m", (token) => {
			...
			return `html+=\`<textarea${attributes(token)}>\${wbpl("${
				valueArg ?? ""
			}", ph) || fieldValue}</textarea>\`;${!nameArg ? "fldCursor();" : ""}`;
		});
...
function formatNumber(value: number, format: string, locale?: string): string {
	const match = format.match(/([^#0.,]*)([#0.,]+)([^#0.,]*)/);
	if (!match) return value.toString();

	const [, prefix, pattern, suffix] = match;
